...
- [ ] **TODO-005**: `--refresh-prices` on `find_plugshare_chargers.py` could not be exercised against the live PlugShare API from the agent's sandbox (network egress to `api.plugshare.com` is blocked there) - needs a real run on the user's machine to confirm it actually backfills rates as intended, not just that it degrades gracefully offline.
